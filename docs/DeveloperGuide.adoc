...
1. User executes the command `add project pr/RAG am/` in the CLI. This input is passed from the `Ui` to `Command` where the input will be parsed to determine the command to execute.
...
. Adds a todo task.
.. Test Case: `add todo d/meeting with MrLim`

+
Expected: Todo task is added and confirmation message is printed.

==== Deadline

. Adds a deadline task.
.. Test Case: `add deadline d/deadline task by/12-11-2019`

+
Expected: Deadline task is added and confirmation message is printed.

==== Snooze Deadline
. Snoozes a deadline task.
.. Prerequisites: Deadline tasks exists.
.. Test case: `snooze id/1`

+
Expected: Deadline is snoozed by 1 day.

==== Postpone Deadline

. Postpones a deadline task.
.. Prerequisites: Deadline tasks exists.
.. Test case: `postpone id/4 n/10`

+
Expected: Deadline is postpone by 10 days.

[NOTE]
More commands can be found on the User Guide.